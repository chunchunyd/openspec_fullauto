...
本规格用于定义"能干"项目的 Redis 缓存基础设施与分层管理规则。

该规格必须支持：

- 统一的 Redis 共享运行时接入层，供所有业务模块复用
- 标准化的 Redis 键命名规范，支持环境隔离、领域分层和版本演进
- 通用的缓存原语，支撑短期状态、计数器和基础频控场景
- 业务缓存语义的本地 ownership，避免全局 key 大杂烩
- 跨运行时共享缓存契约的提取规则
