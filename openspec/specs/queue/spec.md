...
This document defines the queue capability for the Nenggan system, establishing BullMQ as the unified queue runtime layer for async task processing.
...
┌─────────────────────────────────────────────────────────────┐
│                    libs/queue                               │
│  BullMQ 运行时层 - 连接配置、QueueModule、Producer 封装    │
└─────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────────────────────────────────────┐
│                packages/job_contracts                       │
│  任务契约层 - queue name、job name、payload contract       │
└─────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┴─────────────────────┐
        │                                           │
...
|----------|-------------|---------|
| `REDIS_URL` | Redis connection URL for queue backend | `redis://localhost:6379` |

## Related

- [Repository Structure - Queue](../repository-structure/spec.md) - Structural requirements for queue-related directories
